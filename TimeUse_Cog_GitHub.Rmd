---
title: "Twenty-four-hour time-use composition and cognitive function in older adults:
  Cross-sectional findings of the ACTIVate study"
author: "Maddison Mellow"
date: "2022-11-28"
output:
...
The following analysis pipelines were used for our study, "24-hour time-use composition and cognitive function in older adults: cross-sectional findings of the ACTIVate study" (https://www.frontiersin.org/articles/10.3389/fnhum.2022.1051793/full). 

## Data overview

The code presented here was replicated for each cognitive outcome (global cognition, long-term memory, short-term memory, executive function and processing speed). Only the code used for the global cognition outcome is presented here, for simplicity. 

The following variables were included in analyses:

**Cognitive outcome measure**

- `ace_score` = measure of global cognition (Addenbrooke's Cognitive Examination III total score)
...
- `smoke_stat_ch` = smoking status, 3 levels (current, previous or never)
- `edu_yrs` = total years education (primary + secondary + tertiary)


**Interaction terms**

- `recpa_ch` = time spent in recreational physical activity per day, 3 levels (none, 0-30 minutes/day, >30 minutes/day)
- `tvtime_ch` = time spent watching TV per day, 3 levels (low, medium, high tertiles)
...
```
Combine `cols_outcome`, `cols_covar` and `cols_pred` in to `cols_want` dataframe
```{r, eval=FALSE}
cols_want <- c(cols_outcome, cols_pred, cols_covar)
```
Create new dataframe '`act_dat`' which contains all rows from `final_dat` but only `cols_want` columns
```{r, eval=FALSE}
act_dat <- final_dat[, cols_want]

```
Create new vector `rs_ad0`, then create new data frame `index_dont_want` which only includes NAs from `rs_ad0` dataframe
```{r, eval=FALSE}
...
```
Compute ilrs of all rows, but only columns in `cols_pred` (time use variables)
```{r, eval=FALSE}
...
```
- Model 2 = global cognition ~ covariates (age, sex, site, smoking status, education) + interaction data (sleep quality, TV watching time, recreational PA time)
```{r, eval=FALSE}
...
***Model 3 investigates the primary research question of the manuscript (is global cognition associated with 24-hour time-use composition (after adjusting for covariates)). 
...
Set up second series of linear regression models, which each add an interaction term with respective time-use context measures:

- Model 4 = global cognition ~ Model 3 + (time-use composition * sleep quality)
...
```
- Model 5 = global cognition ~ Model 3 + (time-use composition * TV watching time)
...
- Model 6 = global cognition ~ Model 3 + (time-use composition * recreational PA time)
```{r, eval=FALSE}
...
Backwards selection of final models (please see Methods: Statistical Analysis section of manuscript for details on this process)
```{r, eval=FALSE}
car::Anova(ilr_mod_6)
car::Anova(ilr_mod_5)
car::Anova(ilr_mod_4)
...
- This requires the p-values from the final ANOVA output to be extracted and entered in to a dataframe `pvals`
